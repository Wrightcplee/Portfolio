...
class BinarySearchTree(Generic[K, I]):
    ...
    def __getitem__(self, key: K) -> I:
        ...
        """
        return self.get_tree_node_by_key(key)
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        ...
        Complexity: Same as get_miminal
        """
    ...
    def get_minimal(self, current: TreeNode) -> TreeNode:
        ...
        Arguments: current - TreeNode of the root of subtree
        Returns: smallest key in tree

        Best Complexity: O(1), root of subtree does not have a left node.
        Worst Complexity: O(D), D being depth of subtree
        """
        if current is None:
            return None
    ...
    def kth_smallest(self, k: int, current: TreeNode) -> TreeNode:
        ...
        Arguments:
        k - int of the k-th smallest node you want to find
        current - root of subtree

        Returns: TreeNode of k-th smallest
        
        Best Complexity: O(1), kth smallest value is the root
        Worst Complexity: O(D), D being depth of subtree
        """
        left_treesize = 1 if current.left is None else current.left.subtree_size+1
        #left_treesize includes current Node
        if k < left_treesize:
            return self.kth_smallest(k, current.left)
        elif k > left_treesize:
            return self.kth_smallest(k - left_treesize, current.right)
        elif k == left_treesize:
            return current