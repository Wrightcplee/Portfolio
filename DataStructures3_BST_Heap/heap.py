...
class MaxHeap(Generic[T]):
    ...
    def heapify(self, new_list: list|None = None) -> None:
        if new_list is not None:
            self.max_size = max(self.max_size, len(new_list))
            self.the_array = ArrayR(self.max_size+1)
            self.length = len(new_list)
            for i, item in enumerate(new_list):
                self.the_array[i+1] = item
...
if __name__ == '__main__':
    ...
    import random
    new = [random.randint(0,100) for _ in range(15)]
    print(new)
    heap.heapify(new)
    heap_list = []
