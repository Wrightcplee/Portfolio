...
class MyWindow(arcade.Window):
    ...
    def on_init(self) -> None:
        ...
        Initialises UndoTracker and ReplayTracker
        Complexity: O(1)
        """
    ...
    def on_reset(self) -> None:
        ...
        Resets UndoTracker and ReplayTracker
        Complexity: O(1)
        """
        self.undo = UndoTracker()
        self.replay = ReplayTracker()
    ...
    def on_paint(self, layer: Layer, px: int, py: int) -> None:
        ...
        Complexity: O((self.grid.brush_size)**2)
        """
        size = self.grid.brush_size+1
        lim_x = self.grid.x-1
        lim_y = self.grid.y-1
        action = PaintAction()
        for i in range(size):
            for j in range(size-i):
                if self.grid[max(px-i,0)][min(py+j,lim_y)].add(layer):
                    action.add_step(PaintStep((max(px-i,0), min(py+j,lim_y)), layer))
                if self.grid[max(px-i,0)][max(py-j,0)].add(layer):
                    action.add_step(PaintStep((max(px-i,0), max(py-j,0)), layer))
                if self.grid[min(px+i,lim_x)][min(py+j,lim_y)].add(layer):
                    action.add_step(PaintStep((min(px+i,lim_x), min(py+j,lim_y)), layer))
                if self.grid[min(px+i,lim_x)][max(py-j,0)].add(layer):
                    action.add_step(PaintStep((min(px+i,lim_x), max(py-j,0)), layer))
        self.undo.add_action(action)
        self.replay.add_action(action)

    def on_undo(self) -> None:
        """Called when an undo is requested. Complexity: O(PaintAction.undo_apply())"""
        undo_action = self.undo.undo(self.grid)
        self.replay.add_action(undo_action, True)

    def on_redo(self) -> None:
        """Called when a redo is requested. Complexity: O(PaintAction.redo_apply())"""
        redo_action = self.undo.redo(self.grid)
        self.replay.add_action(redo_action)

    def on_special(self) -> None:
        """Called when the special action is requested. Complexity: O(special function of LayerStore)"""
        self.grid.special()
        self.undo.add_action(PaintAction([], True))
        self.replay.add_action(PaintAction([], True))
    ...
    def on_increase_brush_size(self) -> None:
        """Called when an increase to the brush size is requested. Complexity: O(1)"""
        self.grid.increase_brush_size()

    def on_decrease_brush_size(self) -> None:
        """Called when a decrease to the brush size is requested. Complexity: O(1)"""
